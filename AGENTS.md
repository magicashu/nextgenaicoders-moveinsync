...
- For parallel implementation, follow D-037/D-040 and `docs/parallel-delivery-plan.md`: `Java-branch-2` is the integration branch; one Integration Owner freezes shared contracts and dependencies; each worker uses one worktree and writes only its assigned paths; workers do not merge or push directly into `Java-branch-2`.
