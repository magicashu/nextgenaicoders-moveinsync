-- metrics-v1.1 / M14 Sev-1/2 alert rate: severe alerts excluding sign-off violations / trips x 1,000. Grain: trip.
WITH a AS (
    SELECT {{dimension}} AS member, count(*) AS alerts
    FROM alerts
    WHERE business_unit = {{bu}} AND event_date BETWEEN {{start}} AND {{end}} {{filters}} AND NOT is_excluded_event_type AND is_severe
    GROUP BY 1
),
t AS (
