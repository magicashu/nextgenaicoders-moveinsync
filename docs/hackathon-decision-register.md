...
## D-045: Adopt Java-branch-2 implementation on try1

- **Status:** Accepted by explicit user instruction on 2026-09-05.
- **Decision:** Merge `origin/Java-branch-2` at `ee3e755` into `try1`, preserving main ancestry while making the incoming backend, frontend, contracts, migrations, fixtures and supporting configuration authoritative. Do not mix incompatible main-only role/metric DTOs into this runtime.
- **Reason:** Restore the integrated four-role, 18-node deterministic workflow, contextual questions, anomaly detection, approval/resume, mock execution, audit and functional UI already implemented on Java-branch-2.
...
- **Configuration:** Publish the provider adapters, environment-variable template and launcher. Store the user-supplied Sarvam and Langfuse credentials in the ignored local environment file and encrypted GitHub repository secrets. Verified all four configured secret names without retrieving values. The repository is public; secret values are not part of the code commit. Repository secrets require explicit job wiring and do not populate fresh local checkouts automatically.

## D-049: Reuse dashboard captures and coalesce identical requests

- **Status:** Accepted following the user's repeated-Analyse and scale report on 2026-09-05.
- **Decision:** Analyse resolves the saved tenant/date capture instead of always creating a run. Refresh explicitly creates a replacement. Default dashboard entry resolves a capture automatically. Keep the prior successful result visible while refreshing and retain it on failure.
- **Browser:** Persist up to eight captures and eight chart snapshots in tab-scoped session storage (with memory-only fallback). Clear transient busy/error state on reload. Scope captures by tenant, date, demo actor and roles, and charts by tenant, date and data version. Reuse in-flight requests; protect active selection from late responses.
- **Backend:** Cache bounded run-ID references keyed by authorized actor, tenant, roles, persona, dates and data/contract/workflow/prompt versions. Read current approval state from the run repository. Use fixed lock stripes to coalesce identical concurrent captures and explicit refreshes. Cache immutable governed charts separately by tenant/date/data/contract version, after authorization; chart reads do not invoke LLMs.
- **Scale and limits:** Repeated requests execute neither the graph nor chart SQL on cache hits. Existing DuckDB daily aggregates remain the calculation source; no new metric formula or raw-row browser payload is introduced. Process-local caches are bounded accelerators, not distributed persistence. Multi-replica deployment still needs a shared snapshot/job store and durable coordination. The delivered CSV dataset is a captured source, not a real-time ingestion feed.
- **Validation:** Build plus targeted local concurrency and browser verification; keep the previously requested test-suite skip.

## D-050: Persona-first manager workspace and incident responses

- **Status:** Accepted following the user's persona, explanation and incident requests on 2026-09-05.
- **Entry:** Show the problem statement's Transport Manager, Transport & Facilities Head, and Team / Line Manager before mounting pages or requesting data. Select a business unit explicitly; switching persona cancels active UI updates and restores only that role's capture. Reload asks for the persona again while retaining bounded tab-local captures.
- **Views:** Transport Manager receives the operational investigation; Facilities Head additionally sees leadership narrative and governed M09 cost; Line Manager receives deterministic M01/M04/M06 and shift breakdowns through a read-only LangGraph branch, with no specialist LLM investigation, proposed approval or action. Authorization still enforces the existing role permissions.
- **Dataset boundary:** No manager/team/employee assignment directory exists in the supplied data. Business-unit and shift views must not be labelled as authenticated team headcount. Persona selection is a local demo view selector; production requires a trusted authenticated identity gateway and manager scope mapping.
- **Questions and detail:** Reuse current verified findings for covered questions. Present bounded plain-language paragraphs, grouped collapsible evidence and human metric labels. Hide internal graph, agent, model and audit screens by default; explicit VITE_SHOW_DIAGNOSTICS=true keeps developer inspection available. UI visibility is not a security boundary.
- **Scope correction:** Match governed ranking dimensions case-insensitively when selecting affected site/shift members; the adapter emits enum names such as SHIFT_ID. Previously lowercase comparisons dropped known affected locations from action proposals.
- **Incident response:** Show actual approval-backed proposals from retained captures. Retrieve fresh approval state before allowing a decision; support approve, narrow existing multi-site/shift scope and approve, or reject with a note. Preserve backend authorization, revalidation, expiry, idempotency and audit. Server-side edit validation requires the same scope fields and only nonempty subsets of original affected members; metric, monitoring period and other policy fields remain fixed. These actions use the existing mock adapter; response recorded does not mean the physical incident is resolved or an external system changed.
- **Limits:** Captured-report queue is not a complete server-side incident history. Shared distributed snapshots, ingestion refresh, production manager identity and real action integrations remain deployment work. No new metric definitions, thresholds or fictional incident data are introduced.

## D-051: Integrate main UI commits with a guarded floating text and voice chat

- **Status:** Accepted by the user's named-commit request on 2026-09-05.
- **History:** Merge 3bf77e1, which contains fab2e14 and 81d9da4, into the locally checkpointed persona/capture/incident implementation. Resolve incompatible API stubs and historical controller imports against the current runtime.
- **Presentation:** Adopt the incoming floating widget and chart/graph layout updates. Preserve real governed metrics, capture caching, persona gating, approval state and real diagnostic records. Do not display the incoming simulated chat metrics, industry benchmarks, artificial progress or static acceptance claims.
- **Chat:** One bottom-right launcher opens text entry, voice dictation and read-aloud controls. All submitted questions use the current business unit, persona, date and related report. Closing cancels speech; role/date changes clear the conversation and discard stale replies. No auto-send on transcription.
