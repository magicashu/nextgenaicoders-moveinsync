...
| D-057 — Backend implementation and agent consolidation | The user's 2026-09-05 instruction supersedes scaffold-only scope. Implement and integrate all four roles on the latest main checkout, retain one authoritative implementation per role, and use the dataset-only DS-01–DS-20 scenarios plus focused contract/security/resource tests. Project-local skills support each role without acting as runtime permissions. |
| D-058 — Bounded analytical serving | Normalize immutable CSVs once into a version-pinned DuckDB database; order facts by tenant/date, enforce composite trip uniqueness, and materialize additive trip counts for repeated window queries. Keep exact median/P90 computations over eligible fact populations; never average precomputed medians/percentiles. Bound database connections, worker admission, query duration, result cardinality and cache entries. Dynamic scenarios compose registered metrics and filters, not generated SQL. Millions of data rows and millions of concurrent users are distinct workloads; report measured limits rather than claiming unlimited scale. |
| D-059 — Claim authority and safe reporting | Generate direct metric claims from canonical deterministic templates and compare proposed wording to those templates before verification. Preserve metric-family versions separately from the run registry version. Semantic critique may remove or qualify claims but cannot authorize facts. Only approved, scoped internal drafts are produced; effect execution remains behind separate approval/revalidation/audit boundaries. |
| D-060 — Aggregate presentation definitions | M08 window/group output sums eligible capped occupied seats and divides by summed eligible capacity (capacity-weighted occupancy), retaining the original trip counts for quality review; zero/negative capacity and negative/missing occupancy are ineligible. M03 requires an explicit recorded reason variant. M02 VALUE uses mean delay and M12 VALUE uses driver rating for backward compatibility; explicit variants remain preferred. Billing queries contain complete source cycles and disclose when a requested window contains none. These definitions are explicit evaluator inputs, not inferred LLM formulas. |
...
### D-061 — Reconciled source corrections and comparison claims

The current immutable source version `576dca7842d8d7d0f2b37a87c1096bc51c388b22c23d46e792247c24586f9acc` supersedes stale single-office and rounded-cost assumptions. DuckDB source reconciliation finds two vanta-Aus offices (69,801 Cedar Ridge trips; 398 Santa Clara trips) and a vanta-Sea May per-trip cost median of 1,390.34. Q2 remains a billed-distance coverage restriction, not a claim that every distance is zero. Severe-resolution scenarios require an eligible severe population; pinnacle-Slc June is unavailable while catalyst-Bay June supplies the supported case.

Canonical comparison claims may cite two non-overlapping windows only when tenant, data/metric version, metric, statistic, filters and units match. The verifier recomputes the displayed difference; percentages use percentage-point differences. These are descriptive comparisons and do not establish causality. Supervisor planning rejects a budget that cannot fund mandatory comparisons rather than silently removing them. [Implementation review](../evals/backend-implementation-review.md) records actual tests and production boundaries.

### D-062 — Runnable Sarvam provider

Use the single `mobility.ai.provider=sarvam` selector, server-only `SARVAM_API_KEY`, documented `/v1/chat/completions` endpoint and current `sarvam-105b` default. The separate legacy `sarvam.enabled` flag is removed to prevent two provider beans. Supervisor planning and semantic criticism use bounded JSON responses; Investigator calculations and Briefing claim rendering remain deterministic. Validate scope before transmission and reject incomplete/non-JSON output. No automatic HTTP retries duplicate billable calls; agent fallback remains available. Transport contract tests use a local HTTP server; an explicit opt-in live test validates credentials without allowing fallback to hide failure. Official API reference: https://docs.sarvam.ai/api-reference/chat/chat-completions .
