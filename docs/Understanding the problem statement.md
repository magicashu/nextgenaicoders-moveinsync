...
This is the plain-English walkthrough of the problem, agents, nodes and architecture components. The current project contains basic Java structure; individual team members will implement the behavior described here.
...
The Supervisor chooses a bounded investigation plan. The Investigator gathers governed evidence. The Evidence Critic challenges unsupported conclusions. The Briefing/Action agent explains verified findings and drafts a recommendation.

[Agents and responsibilities](agents-guide.md) explains each role's input, job, output, limits, fallback and acceptance checks.
...
Read [team work packages](architecture.md#java-scaffold-and-team-ownership) for ownership, dependencies, handoff records and acceptance checks. Read [data and metric rules](requirement.md#official-data-and-metric-contracts) before implementing any measurement.
